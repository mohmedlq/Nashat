...
export default function Report({
  ...
  const waitForDocumentFonts = async () => {
    if ('fonts' in document) {
      try {
        ...
        // لا نوقف الطباعة إذا كان المتصفح لا يدعم FontFaceSet بشكل كامل.
      }
    }
  };

  /* ============================
   * PRINT — صفحة A4 واحدة بدون صفحة ثانية
   * ============================ */
  ...
  const fitReportToPrintPage = async () => {
    ...
    // نقيس التقرير أولاً بعرض A4 الحقيقي بدون أي transform.
    element.style.transform = 'none';
    element.style.width = '210mm';
    element.style.maxWidth = 'none';
    element.style.transformOrigin = 'top right';

    // إجبار المتصفح على إعادة حساب الـ layout قبل أخذ القياسات.
    ...
    if (!widthPx || !naturalHeightPx || !pageHeightPx) return;

    const scale = Math.min(1, pageHeightPx / naturalHeightPx);

    // نوسّع العرض بنفس نسبة التحجيم ثم نصغّر العنصر.
    // النتيجة النهائية تبقى بعرض 210mm وبارتفاع لا يتجاوز 297mm.
    if (scale < 1) {
  ...
  const handlePrint = async () => {
    ...
    try {
      // نحول كل الحقول إلى نص ثابت ثم ننتظر الصور والخطوط قبل القياس.
      await waitForNextPaint();
      await waitForNextPaint();
      ...
      // بعد اكتمال القياس فعلياً فقط نفتح نافذة الطباعة.
      await waitForNextPaint();
      window.print();
    } catch (error) {
  ...
  // ============================
  // منطقة القص التي ستظهر في PNG / PDF
  // ============================
  ...
  return html2canvas(element, {
    ...
    // حجم المنطقة من نقطة البداية إلى النهاية
    width: cropWidth,
    height: cropHeight,
   ...
   * reportRef contains ONLY:
   * Header → Ministry logo → Region → Ministry → Report content → Evidence → Footer
   * The same DOM is used for Print, PDF and PNG.
   * ============================ */

  const enterExportMode = async () => {
    setIsExportMode(true);

    // نلغي أي تحجيم سابق للطباعة قبل التقاط PDF / PNG.
    const element = reportRef.current;
    if (element) {
      element.style.transform = 'none';
      element.style.width = '';
 ...
 const downloadBlob = async (blob: Blob, fileName: string) => {
  ...
  // 1. محاولة مشاركة الملف على الجوال
  if (
    isMobileDevice() &&
  ...
  // 4. إذا لم يتعامل المتصفح مع download
  setTimeout(() => {
    window.open(url, '_blank');
  }, 500);

  // لا تحذف الـ URL بسرعة
  setTimeout(() => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // PDF is captured directly from the same reportRef DOM used by PNG and Print.
      const canvas = await captureReportCanvas(reportRef.current, getCaptureScale());
      ...
      // Fit the exact report DOM into one A4 page.
      // Nothing is added before or after reportRef in the PDF.
      ...
      await downloadBlob(pdfBlob, `${getSafeFileName()}.pdf`);
    } catch (error) {
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      // PNG is captured directly from the same reportRef DOM used by PDF and Print.
      const canvas = await captureReportCanvas(reportRef.current, getCaptureScale());
      const imageBlob = await canvasToBlob(canvas, 'image/png');

      await downloadBlob(imageBlob, `${getSafeFileName()}.png`);
    } catch (error) {
    ...
    >
      ...
      <div ref={pageFrameRef} className="report-page-frame">
        ...
        >
          ...
          {/* Mobile print control — deliberately OUTSIDE reportRef */}
